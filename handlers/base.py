...
from configs import ZHIHU_EXPIRES, WEIXIN_EXPIRES, JAQ_EXPIRES, PEDIY_EXPIRES, BASE_URL, WEIXIN_PAGE_COUNT,redisDB
...
class BaseHandler(tornado.web.RequestHandler):
	...
	def render(self, template_name, **kwargs):
		...
		# last work
		redisDB.set(self.key, html)  # 缓存渲染的最终结果
		redisDB.expire(self.key, self.expires)
...
class WeixinBaseHandler(BaseHandler):
	def initialize(self):
		...
		self.wid=self.request.arguments['id'][0]
		self.key = "wx__" + self.wid
